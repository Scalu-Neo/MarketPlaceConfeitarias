...
const verMais = () => { //->Exibe o número de cards de produtos visíveis na tela.
  cardsVisiveis.value += 4;
};

const editar = (id) => {//->Invoca a rota no Laravel para editar o produto
  router.get(route('alterarProduto', id));
};

const deletar = (id) => {//->Invoca a rota no Laravel para remover o produto
...
// Agrupa imagens por produto
const imagensPorProduto = computed(() => {
  const agrupadas = {};
...
// Índices do carrossel por produto
const indicesImagem = ref({});

const imagemAtual = (produtoId) => {//->Exibe/retorna a imagem atual para o carrosel de cards de produto
  const imagens = imagensPorProduto.value[produtoId] || [];
  return imagens[indicesImagem.value[produtoId] || 0];
};

const proximaImagem = (produtoId) => {//->// Avança para a próxima imagem no carrossel do produto.
  const imagens = imagensPorProduto.value[produtoId] || [];
  if (!imagens.length) return;
  indicesImagem.value[produtoId] = (indicesImagem.value[produtoId] + 1) % imagens.length;
};

const imagemAnterior = (produtoId) => {//-> Retorna a imagem atual sendo exibida no carrossel de um produto.
  const imagens = imagensPorProduto.value[produtoId] || [];
  if (!imagens.length) return;
...
onMounted(() => {//->Seta a imagem que será exibida no carrosel
  if(props.produtos && Array.isArray(props.produtos)){
