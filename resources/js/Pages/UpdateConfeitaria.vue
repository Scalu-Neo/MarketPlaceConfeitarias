...
const id = props.confeitaria?.id;

const form = useForm({
  nome: props.confeitaria?.nome || '',
  telefone: props.confeitaria?.telefone || '',
  cep: props.confeitaria?.cep || '',
  rua: props.confeitaria?.rua || '',
  numero: props.confeitaria?.numero || '',
  bairro: props.confeitaria?.bairro || '',
  cidade: props.confeitaria?.cidade || '',
  estado: props.confeitaria?.estado || '',
  latitude: '',
  longitude: '',
});
...
const validarEnviarFormulario = async () => {
    
    let coordenadasValidas = await buscarCoordenadas();
    
    if (coordenadasValidas) {
        form.post(`/updateConfeitaria/${id}`, {
            onSuccess: () => {
                form.reset();
            },
            onError: (errors) => {
...
<template>
    ...
    <div class="formulario-Container">
        <h2>Alterar dados da confeitaria</h2>
        <form @submit.prevent="validarEnviarFormulario">
            ...
            <div class="grupo-telefone-cep-numero-endereco">
                <div class="formulario-grupo-item">
                    ...
                    <input v-model="form.rua" type="text" id="rua" required>
                </div>
                <div class="formulario-grupo-item">
            ...
            <div class="formulario-grupo">
                ...
                <input v-model="form.bairro" type="text" id="bairro" required>
            </div>
            <div class="formulario-grupo">
                <label for="cidade">Cidade</label>
                <input v-model="form.cidade" type="text" id="cidade" required>
            </div>
            <div class="formulario-grupo">
                <label for="estado">Estado</label>
                <input v-model="form.estado" type="text" id="estado" required>
            </div>
